...
var processEntries = function() {
    ...
    invoiceTotal = invoiceSubtotal - discountAmount;
		...
		//NEW CODE if nothing is entered for subtotal, displays 0's
		if (isNaN(invoiceSubtotal) || invoiceSubtotal <= 0) {
			$('subtotal').value = defaultVal.toFixed(2);
			$("percent").value = defaultVal.toFixed(2);
			$("discount").value = defaultVal.toFixed(2);
			$("total").value = defaultVal.toFixed(2);
		}
		else {
